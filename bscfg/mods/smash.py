...
class PlayerSpaz_Smash(bs.PlayerSpaz):
        ...
        if isinstance(m, bs.HitMessage):
            ...
            if self.hitPoints > 0:
                ...
                if self._cursed and damage > 0:
                    bs.gameTimer(50, bs.WeakCall(
                        self.curseExplode, m.sourcePlayer))
    ...
    def oob_effect(self):
        ...
        if self.multiplyer > 1.25:
            blastType = 'tnt'
            radius = min(self.multiplyer * 5, 1)
        else:
            # penalty for killing people with low multiplyer
            blastType = 'ice'
            radius = 1.5

        bs.Blast(position=self.node.position, blastRadius=radius,
                 blastType=blastType).autoRetain()
...
class SuperSmash(bs.TeamGameActivity):
    ...
    def getSettings(cls, sessionType):
        return [("Time Limit", {'choices': [('None', 0), ('1 Minute', 60), ('2 Minutes', 120),
                ...
                ("Lives (0 = Unlimited)", {
                 'minValue': 0, 'default': 3, 'increment': 1}),
    ...
    def onBegin(self):
        ...
        self._tntDropTimer = bs.Timer(
            1000 * 30, bs.WeakCall(self._dropPowBox), repeat=True)
        self._updateIcons()
            ...
            for team in self.teams:
                if len(team.players) > 1:
                    ...
                    if len(player.gameData['icons']) != 1:
                        print('WTF have', len(
                            player.gameData['icons']), 'icons in non-solo elim')
    ...
    def endGame(self):
        ...
        if not self.timeLimitOnly:
            ...
            results = bs.TeamGameResults()
            for team in self.teams:
                deaths = sum([0 - player.gameData['lives']
                             for player in team.players])
